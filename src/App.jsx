...
function BendaharaView({ activeTab, setActiveTab, users, savings, settings, updateTable, showToast }) {
  ...
  <tr className="bg-yellow-50 text-yellow-950 font-bold uppercase border-b border-yellow-100">
    ...
    <th className="p-4 text-center">Tanggal Pembayaran Terakhir</th>
...
{santriList.map(s => {
  ...
  const lastPayDate = s.historyBayar && s.historyBayar.length > 0 
    ? [...s.historyBayar].sort().reverse()[0] 
    : '-';
  return (
    <tr key={s.id} className="border-b hover:bg-gray-50 transition">
      <td className="p-4 font-bold text-gray-800">{s.name}</td>
      <td className="p-4 text-gray-500 font-semibold">{s.jilid || 'Jilid 1'}</td>
      <td className="p-4 text-center font-semibold text-gray-600">{lastPayDate}</td>
      <td className="p-4 text-center">
        {isPaid ? (
          <span className="inline-flex items-center bg-emerald-50 text-emerald-700 border border-emerald-100 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase">
            <CheckCircle size={11} className="mr-1 text-emerald-600"/> Lunas
          </span>
        ) : (
      ...
      <td className="p-4 text-center">
        <div className="flex flex-col gap-1.5">
          <button 
            ...
            disabled={isPaid}
            className={`font-bold px-3 py-1.5 rounded-xl text-[10px] transition shadow-sm ${
              isPaid 
          ...
          <button 
            onClick={() => alert(`Riwayat Pembayaran ${s.name}:\n${s.historyBayar?.join('\n') || 'Belum ada riwayat pembayaran'}`)}
            className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-3 py-1.5 rounded-xl text-[10px] shadow transition"
          >
            Riwayat Pembayaran
          </button>
        </div>
